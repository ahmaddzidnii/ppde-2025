...
class DataVisualizer:
    ...
    def setup_controls(self):
        """Setup function plotting controls"""
        # Control variables
        ...
        # Frequency control
        self._create_control_group("Frekuensi:", self.frequency, 0.1, 5.0, 0.1, self.on_parameter_change)
        
        # Amplitude control
        self._create_control_group("Amplitudo:", self.amplitude, 0.1, 3.0, 0.1, self.on_parameter_change)
        
        # Phase control
        self._create_control_group("Phase:", self.phase, 0.0, 6.28, 0.1, self.on_parameter_change)

        # Function type
        self._create_combobox("Jenis Fungsi:", self.function_type, 
                             ["sin", "cos", "tan", "exp", "log"], self.on_function_change)

        # Line color
        self._create_combobox("Warna Garis:", self.line_color,
                             ["blue", "red", "green", "orange", "purple", "brown", "pink"], 
                             self.on_style_change)

        # Line style
        self._create_combobox("Style Garis:", self.line_style,
                             ["-", "--", "-.", ":", "solid", "dashed", "dashdot", "dotted"],
                             self.on_style_change)

        # Line width
        self._create_scale("Ketebalan Garis:", self.line_width, 0.5, 5.0, 0.5, self.on_style_change)

    def _create_control_group(self, label_text, variable, from_, to, resolution, command):
        """Helper method to create control group with scale and value label"""
        label = tk.Label(self.control_frame, text=label_text, font=("Arial", 12, "bold"), bg="lightblue")
        label.pack(pady=(20, 5))

        scale = tk.Scale(self.control_frame, from_=from_, to=to, resolution=resolution,
                        orient=tk.HORIZONTAL, variable=variable, command=command,
                        length=200, bg="lightblue")
        scale.pack(pady=5)

        value_label = tk.Label(self.control_frame, text=f"Nilai: {variable.get()}",
                              font=("Arial", 10), bg="lightblue")
        value_label.pack()
        
        # Store reference for updates
        setattr(self, f"{label_text.lower().replace(':', '').replace(' ', '_')}_label", value_label)

    def _create_combobox(self, label_text, variable, values, command):
        """Helper method to create combobox controls"""
        label = tk.Label(self.control_frame, text=label_text, font=("Arial", 12, "bold"), bg="lightblue")
        label.pack(pady=(20, 5))

        combo = ttk.Combobox(self.control_frame, textvariable=variable, values=values,
                            state="readonly", width=18)
        combo.pack(pady=5)
        combo.bind("<<ComboboxSelected>>", command)

    def _create_scale(self, label_text, variable, from_, to, resolution, command):
        """Helper method to create scale controls"""
        label = tk.Label(self.control_frame, text=label_text, font=("Arial", 12, "bold"), bg="lightblue")
    ...
    def setup_data_controls(self):
        """Setup data visualization controls"""
        # Separator
        separator = tk.Frame(self.control_frame, height=2, bg="darkblue")
        separator.pack(fill=tk.X, pady=20)

        # Data section label
        data_label = tk.Label(self.control_frame, text="VISUALISASI DATA", 
                              font=("Arial", 14, "bold"), bg="lightblue")
        data_label.pack(pady=10)

        # Load data button
        load_btn = tk.Button(self.control_frame, text="Load Data CSV", font=("Arial", 11, "bold"),
                            command=self.load_data, bg="orange", fg="white", width=20)
        load_btn.pack(pady=5)

        # Generate sample data button
        sample_btn = tk.Button(self.control_frame, text="Generate Sample Data", font=("Arial", 11),
                              command=self.generate_sample_data, bg="green", fg="white", width=20)
        ...
        self._create_combobox("Jenis Plot:", self.plot_type,
                             ["line", "scatter", "bar", "histogram", "box"], 
                             self.on_plot_type_change)

        # Multiple subplot checkbox
        self.use_subplot = tk.BooleanVar(value=False)
        subplot_check = tk.Checkbutton(self.control_frame, text="Gunakan Multiple Subplot",
                                      variable=self.use_subplot, command=self.toggle_subplot,
                                      font=("Arial", 10), bg="lightblue")
        subplot_check.pack(pady=10)

        # Real-time simulation controls
        self._setup_realtime_controls()

    def _setup_realtime_controls(self):
        """Setup real-time simulation controls"""
        realtime_label = tk.Label(self.control_frame, text="Real-time Simulation:",
                                 font=("Arial", 12, "bold"), bg="lightblue")
        realtime_label.pack(pady=(20, 5))

        self.realtime_active = tk.BooleanVar(value=False)
        self.realtime_btn = tk.Button(self.control_frame, text="Start Real-time", font=("Arial", 11),
                                     command=self.toggle_realtime, bg="red", fg="white", width=20)
        self.realtime_btn.pack(pady=5)

        # Update speed control
        speed_label = tk.Label(self.control_frame, text="Update Speed (ms):",
                              font=("Arial", 10), bg="lightblue")
        speed_label.pack(pady=(10, 2))

        self.update_speed = tk.IntVar(value=500)
        speed_scale = tk.Scale(self.control_frame, from_=100, to=2000, resolution=100,
                              orient=tk.HORIZONTAL, variable=self.update_speed,
                              length=180, bg="lightblue")
        speed_scale.pack(pady=5)

    def setup_export_controls(self):
        """Setup export and save controls"""
        # Separator
        export_separator = tk.Frame(self.control_frame, height=2, bg="darkblue")
        export_separator.pack(fill=tk.X, pady=20)

        # Export section label
        export_label = tk.Label(self.control_frame, text="EXPORT & SAVE",
                               font=("Arial", 14, "bold"), bg="lightblue")
        export_label.pack(pady=10)

        # Save plot button
        save_btn = tk.Button(self.control_frame, text="Save Plot", font=("Arial", 11, "bold"),
                            command=self.save_plot, bg="purple", fg="white", width=20)
        save_btn.pack(pady=5)

        # Export format selection
        self.export_format = tk.StringVar(value="png")
        self._create_combobox("Format:", self.export_format,
                             ["png", "jpg", "pdf", "svg", "eps"], None)

        # DPI setting
        dpi_label = tk.Label(self.control_frame, text="Resolusi (DPI):",
                            font=("Arial", 10), bg="lightblue")
        dpi_label.pack(pady=(10, 2))

        self.dpi_setting = tk.IntVar(value=300)
        dpi_scale = tk.Scale(self.control_frame, from_=100, to=600, resolution=50,
                            orient=tk.HORIZONTAL, variable=self.dpi_setting,
                            length=180, bg="lightblue")
        dpi_scale.pack(pady=5)

        # Export data button
        export_data_btn = tk.Button(self.control_frame, text="Export Data CSV", font=("Arial", 11),
                                   command=self.export_data_csv, bg="brown", fg="white", width=20)
    ...
    def setup_interactive_features(self):
        ...
        interactive_check = tk.Checkbutton(self.control_frame, text="Mode Interaktif (Click to Annotate)",
                                          variable=self.interactive_mode, command=self.toggle_interactive,
                                          font=("Arial", 10), bg="lightblue", wraplength=200)
        interactive_check.pack(pady=10)

        # Clear annotations button
        clear_btn = tk.Button(self.control_frame, text="Clear Annotations", font=("Arial", 10),
                             command=self.clear_annotations, bg="gray", fg="white", width=20)
        clear_btn.pack(pady=5)

    def setup_theme_controls(self):
        """Setup theme and style controls"""
        # Theme section
        theme_label = tk.Label(self.control_frame, text="TEMA & STYLE",
                              font=("Arial", 14, "bold"), bg="lightblue")
        theme_label.pack(pady=(20, 10))

        # Plot style
        self.plot_style = tk.StringVar(value="default")
        self._create_combobox("Plot Style:", self.plot_style,
                             ["default", "ggplot", "seaborn", "classic", "dark_background"],
                             self.on_plot_style_change)

        # Color scheme
        self.color_scheme = tk.StringVar(value="default")
        self._create_combobox("Color Scheme:", self.color_scheme,
                             ["default", "viridis", "plasma", "inferno", "cool", "warm"],
                             self.on_color_scheme_change)

        # Grid style
        self.show_grid = tk.BooleanVar(value=True)
        grid_check = tk.Checkbutton(self.control_frame, text="Show Grid", variable=self.show_grid,
                                   command=self.update_grid, font=("Arial", 10), bg="lightblue")
    ...
    def setup_status_bar(self):
        ...
        # Status label
        self.status_label = tk.Label(self.status_bar, text="Ready", bg="gray", fg="white",
                                    font=("Arial", 10))
        self.status_label.pack(side=tk.LEFT, padx=10, pady=5)

        # Performance info
        self.perf_label = tk.Label(self.status_bar, text="", bg="gray", fg="white",
                                  font=("Arial", 10))
        self.perf_label.pack(side=tk.RIGHT, padx=10, pady=5)

    # Event handlers
    def on_parameter_change(self, value=None):
        """Handle parameter changes for function plotting"""
        # Update value labels
        if hasattr(self, 'frekuensi_label'):
            self.frekuensi_label.config(text=f"Nilai: {self.frequency.get()}")
        if hasattr(self, 'amplitudo_label'):
            self.amplitudo_label.config(text=f"Nilai: {self.amplitude.get()}")
        if hasattr(self, 'phase_label'):
            self.phase_label.config(text=f"Nilai: {round(self.phase.get(), 2)}")
    ...
    def on_plot_style_change(self, event=None):
    ...
    def plot_data_advanced(self):
        """Advanced data plotting with multiple types"""
        ...
        try:
            if plot_type == "line":
                self._plot_line(numeric_columns, date_columns, colors)
            elif plot_type == "scatter":
                self._plot_scatter(numeric_columns, colors)
            elif plot_type == "bar":
                self._plot_bar(numeric_columns, colors)
            elif plot_type == "histogram":
                self._plot_histogram(numeric_columns, colors)
            elif plot_type == "box":
                self._plot_box(numeric_columns, colors)
            ...
            # Update performance info and status
    ...
    def _plot_line(self, numeric_columns, date_columns, colors):
        """Create line plot"""
        if len(date_columns) > 0 and len(numeric_columns) > 0:
            ...
            for i, col in enumerate(numeric_columns[:3]):
                ...
                self.ax.plot(self.current_data[date_col], self.current_data[col], 
                           color=color, label=col, linewidth=2)
            self.ax.set_xlabel('Tanggal')
            self.ax.legend()
        elif len(numeric_columns) >= 1:
            col = numeric_columns[0]
            self.ax.plot(self.current_data[col], linewidth=2, color=colors[0] if colors else 'blue')
            self.ax.set_xlabel('Index')
            self.ax.set_ylabel(col)

    def _plot_scatter(self, numeric_columns, colors):
        """Create scatter plot"""
        if len(numeric_columns) >= 2:
            x_col, y_col = numeric_columns[0], numeric_columns[1]
            self.ax.scatter(self.current_data[x_col], self.current_data[y_col],
                          alpha=0.6, s=50, c=colors[0] if colors else 'blue')
            self.ax.set_xlabel(x_col)
            self.ax.set_ylabel(y_col)

    def _plot_bar(self, numeric_columns, colors):
        """Create bar plot"""
    ...
    def _plot_histogram(self, numeric_columns, colors):
        """Create histogram"""
        if len(numeric_columns) >= 1:
            col = numeric_columns[0]
            self.ax.hist(self.current_data[col].dropna(), bins=30, alpha=0.7, 
                        color=colors[0] if colors else 'lightgreen', edgecolor='black')
            self.ax.set_xlabel(col)
            self.ax.set_ylabel('Frekuensi')

    def _plot_box(self, numeric_columns, colors):
        """Create box plot"""
        if len(numeric_columns) >= 1:
    ...
    def toggle_interactive(self):
        """Toggle interactive annotation mode"""
    ...
    def on_plot_click(self, event):
        """Handle plot click for annotations"""
        ...
        annotation = self.ax.annotate(
            annotation_text, xy=(x, y), xytext=(10, 10), textcoords='offset points',
    ...
    def setup_subplot(self):
        ...
        colors = self.get_colors(n_plots)
        ...
        for i in range(n_plots):
            ...
            if i % 4 == 0:  # Line plot
                ax.plot(self.current_data[col], color=color, linewidth=1.5)
                ax.set_title(f'Line: {col}')
            elif i % 4 == 1:  # Histogram
                ax.hist(self.current_data[col].dropna(), bins=20, alpha=0.7, color=color)
                ax.set_title(f'Histogram: {col}')
            elif i % 4 == 2:  # Box plot
                ax.boxplot(self.current_data[col].dropna())
                ax.set_title(f'Box: {col}')
            else:  # Scatter with first column
                if len(numeric_columns) > 1:
                    ax.scatter(self.current_data[numeric_columns[0]], 
                             self.current_data[col], alpha=0.6, color=color)
